...
function build_pants_gunicorn(){
  ...
  if ! grep -q "gunicorn" $dir/requirements.txt; then
    echo "gunicorn" >> $dir/requirements.txt
  fi
}
...
if [[ -e $e ]]; then
  build_pants_gunicorn $e
else
  build_pants_gunicorn $f
fi
